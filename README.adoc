...
The generated id resolvers use `d/entity` to fetch the data - but this only works for top-level entities. Which is fine if you only use it with data inserted via save-form etc, which will break any data into quadruplets and insert even nested entities as top entities. But it breaks if you insert data manually, not using quadruplets, as in here:

```clojure
@(d/transact *conn* {:tx-data [{:id [::person/id "ann"]
                                     ::person/id "ann"
                                     ::person/addresses [{::address/id "a-one" ; <- nested entity!
                                                          ::address/street "First St."}
                                                         {::address/id "a-two"
                                                          ::address/street "Second St."}]}]})
```

Here the addresses will become nested entities and `(d/entity *conn* [::address/id "a-one"])` will return `nil`. **FIXME**
You can still get the data because `(d/entity *conn* [::person/id "ann"])` will return them (as of Asami 2.3.2 specifying `nested? false` has surprisingly no effect on that).

NOTE: **WIP** The only problem is that https://clojurians.slack.com/archives/C87NB2CFN/p1674348472055449?thread_ts=1674176469.135179&cid=C87NB2CFN[in Pathom 3 v.2022.10.19-alpha this nested data is thrown away]. I'm currently looking into this
